...
interface JamKhususCardProps {
  jamKhusus: JamKhususConfig;
...
export const JamKhususCard: React.FC<JamKhususCardProps> = ({
  jamKhusus,
  dataJamUmum,
  canUpdate,
  canDelete,
  ...
  const badge = getTipeBadge(jamKhusus.tipe);
  
  // Ambil waktu dari jam umum
  const jamUmumReference = dataJamUmum.find(
    j => j.raw.lembaga_id === jamKhusus.lembaga_id && j.urutanJam === jamKhusus.urutan_jam
  );
    ...
    <Card className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 border border-gray-150 hover:shadow-md transition-shadow bg-white rounded-xl">
      <div className="flex flex-wrap items-center gap-3">
        <div className="w-10 h-10 bg-blue-50 text-[#243B7A] rounded-xl flex items-center justify-center font-bold text-sm shrink-0 border border-blue-100/50">
          {jamKhusus.urutan_jam}
        </div>
        <div>
          <div className="flex items-center gap-2">
            <span className="text-base font-bold text-gray-800">
              Hari {jamKhusus.hari} - Jam Ke-{jamKhusus.urutan_jam}
            </span>
            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${badge.bg}`}>
              {badge.icon}
              {jamKhusus.tipe} (Khusus)
            </span>
          </div>
          <p className="text-sm font-medium text-gray-600 mt-1">
            {jamUmumReference ? `${jamUmumReference.jamMulai}–${jamUmumReference.jamSelesai} WIB` : "Waktu tidak ditemukan (Jam Umum telah dihapus)"}
          </p>
