...
export function JurnalDeleteModal({
  isOpen,
  onOpenChange,
  selectedJurnal,
  isPending,
  onDelete
}: JurnalDeleteModalProps) {
  return (
      ...
      <DialogContent className="sm:max-w-md p-6 rounded-[24px] border-none shadow-xl bg-white">
        ...
        <div className="py-2 space-y-4">
          <p className="text-sm text-slate-700 leading-relaxed">
            Apakah Anda yakin ingin menghapus Jurnal Mengajar <span className="font-bold">{selectedJurnal?.mapel}</span> (Pert. {selectedJurnal?.pertemuan_ke})?
          </p>

          <div className="bg-red-50 border border-red-100 rounded-xl p-4">
            <h4 className="text-sm font-semibold text-red-800 mb-2">Peringatan Penghapusan Data!</h4>
            <p className="text-xs text-red-700 leading-relaxed">
              Menghapus Jurnal ini juga akan menghapus semua data yang terkait di dalamnya. Data berikut kemungkinan besar akan <strong>ikut terhapus</strong>:
            </p>
            <ul className="list-disc list-inside text-xs text-red-700 space-y-1.5 ml-1 mt-2">
              <li>Seluruh <strong>Catatan Jurnal</strong> ini</li>
              <li>Seluruh data <strong>Absensi Siswa</strong> pada pertemuan ini</li>
            </ul>
          </div>
