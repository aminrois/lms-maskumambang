...
import { ChevronRight, Loader2, Clock, BookOpen, Sparkles, Check, FileText, AlertCircle, Lock } from "lucide-react";
            ...
            {currentStep === 2 && (
                <div className="space-y-5">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        {completedPertemuans.length > 0 && (
                            <div className="text-xs font-semibold text-slate-600 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200 shrink-0 self-start sm:self-auto">
                                Terisi: <strong className="text-indigo-600">{completedPertemuans.length}</strong> / {maxPertemuan} Pertemuan
                            </div>
                        )}
                    </div>
                    ...
                    {isLoadingCompleted ? (
                        <div className="flex items-center justify-center py-8 text-slate-500">
                            ...
                            <span className="text-xs font-medium">Memeriksa status absensi pertemuan...</span>
                        </div>
                    ) : (
                        <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 gap-2.5 py-2">
                            {Array.from({ length: maxPertemuan }, (_, i) => i + 1).map((n) => {
                                const isLocked = completedPertemuans.includes(n);
                                ...
                                return (
                                    <button
                                        ...
                                        onClick={() => {
                                            if (isLocked) {
                                                toast.error(`Pertemuan ${n} sudah selesai diisi dan terkunci.`);
                                                return;
                                            }
                    ...
                    <div className="flex items-center gap-4 text-xs text-slate-500 pt-1 flex-wrap">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span>Tersedia (Belum Absensi)</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                        ...
                        <div className="flex items-center gap-1.5">
                            <span className="w-3 h-3 rounded-md bg-slate-100 border border-slate-300 inline-flex items-center justify-center">
                                <Lock className="w-2 h-2 text-slate-500" />
                            </span>
                            <span>Terkunci (Sudah Diisi)</span>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between pt-4 border-t border-slate-100">
                        ...
                        <Button
                            onClick={nextStep}
                            disabled={!selections.pertemuan || completedPertemuans.includes(selections.pertemuan)}
                            className="bg-[#243B7A] hover:bg-[#1a2b5a] text-white rounded-xl px-6"
                        >
