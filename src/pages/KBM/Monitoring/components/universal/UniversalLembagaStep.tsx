...
interface UniversalLembagaStepProps {
  lembagas: LEMBAGA[];
  onSelectLembaga: (id: string) => void;
  setStep: (step: 'select_lembaga' | 'select_kelas' | 'display_results') => void;
}

export function UniversalLembagaStep({
  lembagas,
  onSelectLembaga,
  setStep
}: UniversalLembagaStepProps) {
  return (
    <div className="space-y-6 max-w-4xl mx-auto pt-6">
      <div className="text-center space-y-2 mb-8">
        ...
        <p className="text-slate-500 text-sm">Pilih lembaga pendidikan untuk memonitor progres KBM.</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {lembagas.map((l) => (
          <button
            key={l.lembaga_id}
            onClick={() => {
              onSelectLembaga(String(l.lembaga_id));
